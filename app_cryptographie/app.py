...
@app.route('/')
def aceuille():
    if os.path.exists('claire.png'):
        os.remove('claire.png')
...
def chiffrement_images():
    if request.method == "POST":
        ...
        if os.path.exists('file.jpg'):
            ...
            os.remove('file.jpg')
        if os.path.exists('hide.png'):
            return send_file('hide.png', as_attachment=True)
...
def dchiffrement_images():
    if request.method == "POST":
        ...
        request.files['image1'].save('file.jpg')
        if os.path.exists('file.jpg'):
            my_steg.decode('file.jpg')
            os.remove('file.jpg')
        if os.path.exists('claire.png'):
            return send_file('claire.png', as_attachment=True)
